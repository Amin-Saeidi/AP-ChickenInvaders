...
public class Level4 implements LevelSample {
	...
	public Level4(MainPanel panel,Level level) {
		this.panel = panel;
		this.level = level;
		wave1 = new Game(1200, 700, "rectangle", "3", "2", "1",panel);
		wave2 = new Game(1200, 700, "circular", "2", "3", "4",panel);
		wave3 = new Game(1200, 700, "rectangle", "4", "3", "1",panel);
		wave4 = new Game(1200, 700, "circular", "4", "3", "4",panel);
		finalWave = new Game(1200, 700, "GiantEgg",panel);
		endOfLevelMessage = new  EndOfLevelMessage(250, 0, 600, 200);
	}

	public Level4(Level level) {
		this.level = level;
		wave1 = new Game(1200, 700, "rectangle", "3", "2", "1");
		wave2 = new Game(1200, 700, "circular", "2", "3", "4");
		wave3 = new Game(1200, 700, "rectangle", "4", "3", "1");
		wave4 = new Game(1200, 700, "circular", "4", "3", "4");
	...
	public void endOfWave1(){
		if(Level.loaded != true) {
			if(counter  == 0) {
				wave1.propertiesBar.setHealth(level.level3.finalWave.propertiesBar.getHealth());
				wave1.propertiesBar.setCoins(0);
				wave1.propertiesBar.setChickens(level.level3.finalWave.propertiesBar.getChickens());
				wave1.temperatureBar.setScore(level.level3.finalWave.temperatureBar.getScore()+level.level3.finalWave.propertiesBar.getCoins());
				wave1.temperatureBar.maximumTemp = level.level3.finalWave.temperatureBar.maximumTemp;
				wave1.propertiesBar.setNumOfBombs(level.level3.finalWave.propertiesBar.getNumOfBombs()+1);
				counter+=1;
			}
		}
		...
		if(Level.loaded != true) {
			if(finalWave.giantEgg.paintCheck == false){
				...
				if (endOfLevelMessage.getMessageY()>800) {
					endOfLevelMessage.showMessage=false;
				}	
			}
		}
	}
